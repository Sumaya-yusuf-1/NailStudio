...
function RoundShape({ color, className }: BaseProps) {
   const gradId = "nail-grad-round";
  const fill = getFill(color, gradId);
  return (
    <svg
      viewBox="0 0 39 82"
      className={className}
    ...
    >
      <GradientFill color={color} id={gradId} />
...
function GradientFill({ color, id }: { color: string; id: string }) {
  if (!isLinearGradient(color)) return null;

  const stops = extractHexColors(color);
...
function getFill(color: string, id: string) {
  return isLinearGradient(color) ? `url(#${id})` : color;
}
